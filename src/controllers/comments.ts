...
export async function getAllComments(): Promise<PaginatedResponse<Comment>> {
    try {
      const data = await fs.promises.readFile(path.join(process.cwd(), 'src/database/comments.json'), 'utf8');
...
// get by limit and skip
  export async function getCommentsByLimitAndSkip(skip: number = 0, limit: number = 30): Promise<PaginatedResponse<Comment>> {
    try {
      const data = await fs.promises.readFile(path.join(process.cwd(), 'src/database/comments.json'), 'utf8');
      const comments: Comment[] = JSON.parse(data);
